...
class ValidatorTrackingDataFactory(factory.Factory):
    class Meta:
        model = ValidatorTrackingData

    @classmethod
    def create_batch_with_overrides(cls, **kwargs) -> ValidatorTrackingData:
        """
    ...
    sale_amount = factory.Faker("pydecimal", left_digits=5, right_digits=2, positive=True)

    device = factory.LazyFunction(lambda: random.choice(list(Device)).value)
    created_at = factory.LazyAttribute(lambda _: str(datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")))
    updated_at = factory.LazyAttribute(lambda _: str(datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")))
