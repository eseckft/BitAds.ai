    ...
    def create_dirs(self, type):
        ...
        for path in directories:
            if not os.path.exists(path):
                os.makedirs(path)
                Hint(Hint.COLOR_MAGENTA, Const.LOG_TYPE_LOCAL, f"Created directory: {path}")

    def saveCampaign(self, wallet_address, type, task):
        path =  self.DIR_ROOT + "/" + type + "/" + wallet_address + "/campaign_id/" + task['product_unique_id'] + ".json"
        with open(path, 'w') as file:
            json.dump(task, file, indent=4)
            Hint(Hint.COLOR_YELLOW, Const.LOG_TYPE_LOCAL, "Save campaign to: " + path)

    def removeCampaign(self, wallet_address, type, campaignId):
        path =  self.DIR_ROOT + "/" + type + "/" + wallet_address + "/campaign_id/" + campaignId + "_" + wallet_address + ".json"
        if os.path.exists(path):
            os.remove(path)
            Hint(Hint.COLOR_YELLOW, Const.LOG_TYPE_LOCAL, "Remove campaign file: " + path)

    def saveMinerUniqueUrl(self, owner, wallet_address, type, data):
        path =  self.DIR_ROOT + "/" + type + "/" + owner + "/unique_link/" + data['product_unique_id'] + "_" + wallet_address + ".json"
        with open(path, 'w') as file:
            json.dump(data, file, indent=4)
            Hint(Hint.COLOR_YELLOW, Const.LOG_TYPE_LOCAL, "Save campaign unique url to: " + path)

    def saveMinerUniqueUrlStats(self, owner, uniqueUrl, type, data):
        path =  self.DIR_ROOT + "/" + type + "/" + owner + "/unique_link_stats/" + data['product_unique_id'] + "_" + uniqueUrl + ".json"
        with open(path, 'w') as file:
            json.dump(data, file, indent=4)
            Hint(Hint.COLOR_YELLOW, Const.LOG_TYPE_LOCAL, "Save campaign unique url statistics to: " + path)

    def saveMinerUniqueUrlScore(self, owner, uniqueId, uniqueUrl, type, data):
        path =  self.DIR_ROOT + "/" + type + "/" + owner + "/unique_link_score/" + uniqueId + "_" + uniqueUrl + ".json"
        with open(path, 'w') as file:
            json.dump(data, file, indent=4)
            Hint(Hint.COLOR_YELLOW, Const.LOG_TYPE_LOCAL, "Save campaign unique url score to: " + path)

    def unique_link_exists(self, owner, wallet_address, type, campaignId):
        path =  self.DIR_ROOT + "/" + type + "/" + owner + "/unique_link/" + campaignId + "_" + wallet_address + ".json"
        if not os.path.exists(path):
            return False
